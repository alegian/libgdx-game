...
public class Repairman implements IPlaceable{
    /**
     ...
     * @param x the repairman's home X position in interpolated Tile Coordinates
     * @param y the repairman's home Y position in interpolated Tile Coordinates
     * @param premium if the repairman is the gold version or not
     */
    public Repairman (HuntersGame game, World world, int x, int y, boolean premium){
        try {
            this.game = game;
            this.world = world;
            ...
            type = PlaceableType.REPAIRMAN;
            texture = new Texture("repairman_human.png");
            if (premium) {
                type = PlaceableType.REPAIRMAN_PREMIUM;
            }
        }catch(Exception ex){
    ...
    public PlaceableType getType() {
        if(premium) return PlaceableType.REPAIRMAN_PREMIUM;
        return PlaceableType.REPAIRMAN;
    }
    /**
    ...
    private void update(float delta){
        ...
        if(mode == StaffMode.NONE){
            ...
            if(macroDestination==null){
                ArrayList<Building> brokenAttractions = world.getAvailableBrokenAttractions();
                if(brokenAttractions.size()>0){
                    int cost = 10;
                    if (premium) {
                        cost=5;
                    }
                    Economy.subtractMoney(cost);
                    Building randomAttraction = brokenAttractions.get(random.nextInt(brokenAttractions.size()));
                    destinationBuilding=randomAttraction;
                    destinationBuilding.setBeingRepaired(true);
            ...
            if(frameCounter==0){
                ...
                if(destinationBuilding != null){
                    destinationBuilding.setBroken(false);
                    destinationBuilding.setBeingRepaired(false);
                }
     ...
     */
    public void reset() {
        if(destinationBuilding!=null) destinationBuilding.setBeingRepaired(false);
        destinationBuilding = null;
